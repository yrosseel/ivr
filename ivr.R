#
# ivr() function for estimation of the instrumental variables 
#       regression (IVR) model
#
# reference:

# Maydeu-Olivares, A., Shi, D., & Rosseel, Y. (2019). Instrumental variables
# two-stage least squares (2SLS) vs. maximum likelihood structural equation
# modeling of causal effects in linear regression models. Structural Equation
# Modeling: A Multidisciplinary Journal, 26(6), 876-892.
#
...
ivr <- function(y = NULL,              # single outcome
    ...
    # zero block mean/covariance
    Gamma.ADF2 <- Gamma.ADF
    Gamma.ADF2[1:nvar, -(1:nvar)] <- 0
    Gamma.ADF2[-(1:nvar), 1:nvar] <- 0
...
#
    ...
    C.hat.ADF.sample <- ( Delta.c %*% solve(t(Delta.c) %*% Gamma.ADF.sample
                           %*% Delta.c) %*% t(Delta.c) )
    C.hat.ADF2 <- ( Delta.c %*% solve(t(Delta.c) %*% Gamma.ADF2 %*% Delta.c)
                   %*% t(Delta.c) )
...
#
    ...
    T.Browne.ADF2 <- as.numeric( nobs * (t(RES) %*% C.hat.ADF2 %*% RES) )
    ...
    out <- list(m.y = m.y, m.x = m.x, m.z = m.z,
                ...
                T.Browne.ADF = T.Browne.ADF, 
                #T.Browne.ADF.s = T.Browne.ADF.s,
                T.Browne.ADF2 = T.Browne.ADF2, 
                RES = RES,
                meanstructure = meanstructure,
                fixed.x = fixed.x,
